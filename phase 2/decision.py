...
class PID:
    ...
    def get_longitudinal_control(self,currentState,desired_throttle,dt):
        ...
        current_throttle=currentState[3]


        e = desired_throttle - current_throttle  
...
def forward(Rover, throttle, desired_thottle,steer, can_move):
    ...
    if can_move:
        if Rover.vel < Rover.max_vel:
            ...
            longitudunal_Control = pid.get_longitudinal_control(np.array([Rover.pos[0],Rover.pos[1],Rover.nav_angles,throttle]),desired_thottle,(1/25))
            ...
            if Rover.mode == 'sample': # state in sample found
                if Rover.sample_angles.any():
                    ...
                    if Rover.near_sample > 0:
                        ...
                        if Rover.vel > 0.2:
                            ...
                            forward(Rover, Rover.throttle,0.2, 0.8*steer, True)
                    elif Rover.sample_angles is not None:
                        # sample is not close so move towards it
                        forward(Rover, Rover.throttle, Rover.throttle_set ,steer*0.8, True)
                    else:
                        Rover.mode = 'turn_around'
                else:
                    Rover.mode = 'turn_around'
                ...
                if len(Rover.nav_angles) >= Rover.stop_forward:
                    if len(Rover.nav_angles) > 0:
                        ...
                        steer = 0
                    forward(Rover, Rover.throttle,Rover.throttle_set,steer*0.9, Rover.can_go_forward)
                else:
                    Rover.mode = 'turn_around'
                    # turn_around(Rover)
            else:
