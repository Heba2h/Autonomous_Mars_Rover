...
def perception_step(Rover):
    ...
    print("hi x_pixel , y_pixel"+ str(x_pixel)+str(y_pixel))
...
# # This code is placed after step 6 
    # copy_of_accumulated_map=Rover.map[:,:,2].copy()




    # print("-----------------------------------")
    # print("Max",np.max(copy_of_accumulated_map))
    # print("Sum",np.sum(copy_of_accumulated_map))
    
    # copy_of_accumulated_map[copy_of_accumulated_map<150]=False
    # copy_of_accumulated_map[copy_of_accumulated_map>=150]=True
    

    # vision=np.zeros_like(copy_of_accumulated_map)

    # index=np.arange(1,len(navigable_x_world)+1)
    # vision[navigable_x_world,navigable_y_world]=index


    # new_vision=np.logical_and(np.clip(vision,0,1),np.logical_not(copy_of_accumulated_map)).astype(int)
    # indexes_real=vision[new_vision*vision!=0].astype(int)

    # x_pix_real,y_pix_real=x_pixel[indexes_real-1].astype(int),y_pixel[indexes_real-1].astype(int)

    # print("Max",np.max(copy_of_accumulated_map))
    # print("Sum",np.sum(copy_of_accumulated_map))
    # print("Old",len(x_pixel))
    # print("Neew",len(x_pix_real))
    # print("-----------------------------------")
   
    # new_threshed=np.zeros_like(threshed)
    # new_threshed[x_pix_real-1,y_pix_real-1]=1
    # new_threshed=np.zeros_like(threshed)
    # new_threshed[x_pix_real,y_pix_real]=255
    # Rover.vision_image[:,:,2]=new_threshed
    # free_pixel=Rover.vision_image[:,:,2]>0
    # Rover.vision_image[free_pixel,0]=0
    # Rover.nav_dists,Rover.nav_angles= to_polar_coords(x_pix_real, y_pix_real)
    # Rover.nav_angles=rover_and_curve_par(new_threshed)
